...
export default function Authorityhome({ email }) {
  ...
  useEffect(() => {
    const fetchEmails = async () => {
      try {
        const pendingResponse = await axios.get('https://ayush-portal-new-backend.vercel.app/api/isfilledapplication-false');
        setpendingStartupEmails(pendingResponse.data.success ? pendingResponse.data.pendingList : []);

        const assignedResponse = await axios.get('https://ayush-portal-new-backend.vercel.app/api/isDrugInspectorAssigned-true');
        setassignedStartupEmails(assignedResponse.data.success ? assignedResponse.data.assignedList : []);

        const acceptedResponse = await axios.get('https://ayush-portal-new-backend.vercel.app/api/isDrugInspectorAccepted-true');
        setacceptedStartupEmails(acceptedResponse.data.success ? acceptedResponse.data.acceptedList : []);

        const rejectedResponse = await axios.get('https://ayush-portal-new-backend.vercel.app/api/isDrugInspectorRejected-true');
        setrejectedStartupEmails(rejectedResponse.data.success ? rejectedResponse.data.rejectedList : []);

        const licensedResponse = await axios.get('https://ayush-portal-new-backend.vercel.app/api/isLicensed-true');
        setlicensedStartupEmails(licensedResponse.data.success ? licensedResponse.data.licensedList : []);

        const lArejectedResponse = await axios.get('https://ayush-portal-new-backend.vercel.app/api/isLArejected');
