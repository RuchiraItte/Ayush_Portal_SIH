 ...
 useEffect(()=>{
   const fetchit = async(e)=>{
    ...
    try{
      if(name='farmer')
        {
      const response=await axios.post('https://ayush-portal-new-backend.vercel.app/api/farmer-dashboard',email);
      //variable name is email but it contains phone number in case of farmer
        }
        else if(name='doctor')
        { 
          const response= await axios.post('https://ayush-portal-new-backend.vercel.app/api/doctor-dashboard',email);
        }
        if(response.success)
        {
