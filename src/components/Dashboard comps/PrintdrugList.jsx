...
const handleSubmit = async(presentmail) => {
  ...
  try {
    const response = await axios.post('https://ayush-portal-new-backend.vercel.app/api/make-it-rejected', { Email_ID: presentmail }); 
    if (response.data.success) {
 ...
 async function acceptstartup(presentmail)
  {
    ...
    try {
      const response = await axios.post('https://ayush-portal-new-backend.vercel.app/api/make-it-accepted', { Email_ID: presentmail }); 
      if (response.data.success) {
