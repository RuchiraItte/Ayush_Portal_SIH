    ...
    setTimeout(() => {
      ...
      setRenderG(<span style={{ color: 'green' }}>Guidelines are validated</span>);
        ...
        const  doVerifyGuidlines = async(formData)=>
          {
            try {
              const response = await axios.post("https://ayush-portal-new-backend.vercel.app/api/guideline-check",
                formData,
                {
                  headers: {
...
const handleFileChange = (e) => {
  setSelectedFile(e.target.files[0]); // Store the selected file in state
    ...
    const handleSubmit= async(e)=>
    {
        ...
        try{
        const response= await axios.post("https://ayush-portal-new-backend.vercel.app/api/drugInspector-reg",drugindata);
        if(response.data.success)
        {
